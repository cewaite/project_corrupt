...
func fire_ray(spread_radius: float) -> Dictionary:
	...
	var random_point = Vector3(randf_range(-spread_radius, spread_radius), randf_range(-spread_radius, spread_radius), randf_range(-spread_radius, spread_radius))
	var end_point = (forward_dir * 1000) + random_point
	var ray_intersect = PhysicsRayQueryParameters3D.create(global_position, end_point)
